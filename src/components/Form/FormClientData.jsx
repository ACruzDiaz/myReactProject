...
const FormClientData = ({buyBag, totalPrice, bag}) => {
  const initialState = {
    nombreIn:"",
    telefonoIn:"",
    direccionIn:"",
  ...
  const rForm = useRef(null);
    ...
    const getTotal = (fees, totalg) => {
      return Number.parseFloat(Number.parseFloat(fees) + Number.parseFloat(totalg)).toFixed(2);
    }
    useEffect(() => {
    ...
    const handleComprar = (e) => {
      ...
      if(empty){
        ...
        (buyBag(clientData,total)).then((res)=>{
  ...
  return(
    <section className='formSection'>
      {bag && bag.length >0 ?
      <><form ref={rForm} className='formShipping' onSubmit={handleComprar}>
        ...
        </form><div className="bagContainerCostos">
          ...
          </div></>: <h1>Su bolsa esta vacia. <br/> Agregue articulos para continuar.</h1>
    }
    </section>)
